...
#define GRASS_TEX_SIZE 420
GLubyte grama[GRASS_TEX_SIZE][GRASS_TEX_SIZE][3];
...
void loadGrass() {
	...
	int i,j;
	for(i = 0; i < GRASS_TEX_SIZE; i++)
		for(j = 0; j < GRASS_TEX_SIZE; j++) {
			int a, b, c;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, GRASS_TEX_SIZE, GRASS_TEX_SIZE, 0, GL_RGB, GL_UNSIGNED_BYTE, grama);
}
...
void display(void)
{
    ...
    glBindTexture(GL_TEXTURE_2D, 2); //ADD
